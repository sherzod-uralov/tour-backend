...
export class ToursService {
  ...
  async create(createTourDto: CreateTourDto): Promise<Tour> {
    ...
    if (
      !createTourDto.lemonSqueezyProductId ||
      !createTourDto.lemonSqueezyVariantId
    ) {
    ...
    if (createTourDto.difficultyId) {
      const difficulty = await this.difficultiesService.findOne(
        createTourDto.difficultyId,
      );
  ...
  private async addAverageRatingsToTours(
    tours: Tour[],
  ): Promise<(Tour & { averageRating?: number })[]> {
    if (tours.length === 0) {
      return [];
    }
  ...
  async findOne(
    id: number,
  ): Promise<Tour & { comments?: TourComment[]; averageRating?: number }> {
  ...
  async remove(id: number): Promise<void> {
    ...
    const commentsCount = await this.tourCommentsRepository.count({
      where: { tourId: id },
    });
    if (commentsCount > 0) {
      throw new ConflictException(
  ...
  async findByCategoryUrl(
    categoryUrl: string,
  ): Promise<(Tour & { averageRating?: number })[]> {
    const category = await this.categoriesService.findByUrl(categoryUrl);
  ...
  async searchTours(
    searchDto: SearchToursDto,
  ): Promise<{
    tours: (Tour & { averageRating?: number })[];
    total: number;
    page: number;
    limit: number;
  }> {
